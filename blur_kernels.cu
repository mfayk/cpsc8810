...
__global__ 
void contrast(uchar4 *d_in, int numRows, int numCols, float *mean, float *variance){

  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
  
  if (y < numRows && x < numCols){
    //variance += pow(d_in[y*numCols+x] - mean, 2);
  }
...
__global__ 
void k_operations_contrast(int rows, int cols, int numclasses, float *centroids,  int *new_predictions, uchar4 *d_img, int *predictions_d, float contrast){
  ...
  if(c < cols && r < rows){
    ...
    {   
        ...
        gray_temp =  0.0722*(float)d_img[r*cols+c].x + 0.7152*(float)d_img[r*cols+c].y + 0.2126*(float)d_img[r*cols+c].z;

        dist = sqrt(pow((gray_contrast - (float)gray_temp),2));
...
free(homogeneous_h);
 ...
 void your_kmeans_contrast(int rows, int cols, int numclasses, int *predictions, vector<float> *centroids, uchar4 *h_img, int max_its, float contrast){
...
/*
  im2Gray<<<gridSize, blockSize>>>(d_img, gray_img, rows, cols, gray_sums);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());

  *mean = *gray_sums/(rows*cols);

  contrast<<<gridSize, blockSize>>>(d_img,rows,cols,mean,variance);
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
...
std::cout << "GPU contrast sanity" << contrast << "\n";

    for(count=0; count<iterations; count++)
    {
        std::cout << "starting iteration " << iteration << "\n";

        k_operations_contrast<<<gridSize, blockSize>>>(rows,cols,numclasses,centroids_d,new_predictions_d,d_img,predictions_d,contrast);
        cudaDeviceSynchronize();
        checkCudaErrors(cudaGetLastError());
        centroid_update<<<gridSize, blockSize>>>(rows,cols,numclasses,centroids_d,new_predictions_d,d_img,predictions_d);
        cudaDeviceSynchronize();
...
void your_kmeans_contrast_shared(int rows, int cols, int numclasses, int *predictions, vector<float> *centroids, uchar4 *h_img, int max_its, float contrast){
